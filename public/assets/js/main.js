...
function initGoogleMap() {
	...
	if ( $googleMaps.length ) {
		$googleMaps.each(function() {
			...
			//markers
			var $markers = $map.find('.map_marker_description');
			...
			geocoder.geocode({
				...
				$($markers).each(function(index) {
					...
					var $marker = $(this);
					var markerTitle = $marker.find('h3').text();

					//building info widnow HTML code
					var markerDescription = '';
					markerDescription += markerTitle ? '<h3 class="makret-title">' + markerTitle + '</h3>' : '';
					markerDescription += $marker.find('p').text() ? '<div class="marker-description">' + $marker.find('p').text() + '</div>' : '';
					...
					geocoder.geocode({
						...
						var iconSrc = $marker.find('img').attr('src');
